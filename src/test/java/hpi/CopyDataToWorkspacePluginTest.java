...
class CopyDataToWorkspacePluginTest {
	...
	/**
	 * Test for setUp with mock FilePath
	 * This test ensures proper mocking of Jenkins environment
	 */
	@Test
	void testSetUpWithMockFilePath() throws Exception {
		// Skip on Windows due to path handling differences
		assumeTrue(!isWindows(), "Skipping mock FilePath test on Windows");
		...
		FilePath userContentDir = mock(FilePath.class);
		FilePath copyFrom = mock(FilePath.class);
		
		// Setup Jenkins mock
		Jenkins jenkins = mock(Jenkins.class);
		when(jenkins.getRootPath()).thenReturn(jenkinsRoot);
		
		// Use reflection to set the Jenkins instance
		Field jenkinsField = Jenkins.class.getDeclaredField("theInstance");
		jenkinsField.setAccessible(true);
		Object oldJenkins = jenkinsField.get(null);
		jenkinsField.set(null, jenkins);
		
		try {
			// Setup FilePath mocks
			when(build.getWorkspace()).thenReturn(workspace);
			when(jenkinsRoot.child("userContent")).thenReturn(userContentDir);
			when(userContentDir.child(TEST_DIR)).thenReturn(copyFrom);
			
			// Set explicit paths to avoid null in paths
			when(jenkinsRoot.getRemote()).thenReturn("/jenkins");
			when(userContentDir.getRemote()).thenReturn("/jenkins/userContent");
			when(copyFrom.getRemote()).thenReturn("/jenkins/userContent/" + TEST_DIR);
			
			when(copyFrom.exists()).thenReturn(true);
			when(copyFrom.list()).thenReturn(Arrays.asList());
			...
			// Call setUp
			hudson.tasks.BuildWrapper.Environment env = plugin.setUp(build, launcher, listener);
			
			// Verify interactions
			verify(copyFrom).copyRecursiveTo(workspace);
			
			// Verify environment is created
			assertNotNull(env, "Environment should be created");
			
			// Call tearDown
			boolean result = env.tearDown(build, listener);
			assertTrue(result, "TearDown should return true");
			
			// Test path outside userContent
			reset(userContentDir, copyFrom);
			when(userContentDir.child("../outside")).thenReturn(copyFrom);
			when(copyFrom.getRemote()).thenReturn("/jenkins/outside");
			
			// Create plugin instance with path outside userContent
			CopyDataToWorkspacePlugin pluginOutside = new CopyDataToWorkspacePlugin(
					"../outside",
					false,
					false
			);
			
			// Call setUp - should throw exception
			IOException exception = assertThrows(IOException.class, () -> {
				pluginOutside.setUp(build, launcher, listener);
			});
			
			assertTrue(exception.getMessage().contains("must be within the JENKINS_HOME/userContent directory"), 
					"Exception should mention userContent directory");
		} finally {
			// Restore the original Jenkins instance
			jenkinsField.set(null, oldJenkins);
		}
	}
